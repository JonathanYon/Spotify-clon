...
// window.onload = () => fechload("shakira")
...
.then(response => {
    ...
    for(let i in response.data){
                resArr[i] = {
                    Title: response.data[i].album.title,
                    title: response.data[i].title,
                    cover: response.data[i].album.cover,
                    name: response.data[i].artist.name,
                    id: response.data[i].album.id,
                    album: response.data[i].album
                    
                }
                
            }
    // console.log("hiiiiiiiii", resArr[0].album)
    let songsRow = document.querySelector(".container-songs")
    resArr.forEach( (album, i) => {
                ...
                <div class="imgg mx-4">
                  <img class="img-thumb" src="${album.cover}" alt="" />
                </div>
                <p class="album-names mb-lg-2 padding-list">${album.Title}</p>
              </div>
              ...
              <div class="col-2 d-flex">
               ...
               ` ) 
                ...
                // window.onload = () => {
                  ...
                  //   albumImg.innerHTML = ""
                  //   albumImg.innerHTML = `<img
                  //   class="main-image"
                  //   aria-hidden="false"
                  //   draggable="false"
                  //   loading="eager"
                  //   src="${resArr.album.cover_medium}"
    ...
    // console.log("heyyyyyyyyyyyy", songsRow)
    
    // const newObj = {    //this is the the same as the abpve exept it holds the name and whole album of the singer as a prop
    //     title: q,
    //     album: response.data
    // }
    // resArr.push(newObj)
    //         console.log("hiiiiiiiii", resArr)
...
// window.onload = () => dataFetch("shakira")
